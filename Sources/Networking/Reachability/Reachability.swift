...
//
    ...
    open class Reachability {
        ...
        private var activeSubscriptionsCount = 0 {
            willSet {
                switch (newValue, activeSubscriptionsCount) {
                case (1, 0):
                    startNotifier()
                case (0, 1):
                    stopNotifier()
        ...
        public required init(
            ...
            // observe subscriptions to start/stop notifier
            startObserving()
        }
    ...
    private extension Reachability {
        func startObserving() {
            connection.map { _ in }
                .merge(with: isReachable.map { _ in })
                .merge(with: isConnected.map { _ in })
                .merge(with: isDisconnected.map { _ in })
                .handleEvents(
